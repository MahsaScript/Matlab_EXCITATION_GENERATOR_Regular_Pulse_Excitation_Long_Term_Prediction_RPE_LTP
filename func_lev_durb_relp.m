...
tcount_of_aCoeff = length(aCoeff);
 ...
 y_padded_for_delay_removal = [y; zeros(1,1)]; %it is padded with zeros to remove the effect of delay in filter
 est_y_with_dummy_pad = filter([0 -aCoeff(2:end)],1,y_padded_for_delay_removal);    % = s^(n) with a cap on page 92 of the book
est_y = est_y_with_dummy_pad(2:end);    
